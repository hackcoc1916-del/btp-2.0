...
logging.basicConfig(
    filename=os.path.join(LOGS_DIR, "04_random_forest.log"),
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(message)s"
)

def train_rf(task_type, parquet_file, scaler):
    ...
    print(f"Training Random Forest ({task_type}) on shape {X_train.shape} (GPU not required) ...")
    ...
    model_out = os.path.join(MODELS_DIR, f"random_forest_{task_type}.pkl")
    with open(model_out, "wb") as f:
        pickle.dump(model, f)
    print(f"Saved model to {model_out}")
    
    logging.info(f"Successfully trained random_forest_{task_type}. Validation Accuracy: {acc:.6f}")
    return model

def main():
    print("="*60)
    print(" STAGE 3: RANDOM FOREST BASELINE TRAINING ")
    print("="*60)
    ...
    model_multi = train_rf("multiclass", "cic2017_multiclass.parquet", scaler)
    model_bin = train_rf("binary", "cic2017_binary.parquet", scaler)
    
    # Save combined dictionary to random_forest.pkl as requested by user
    combined_out = os.path.join(MODELS_DIR, "random_forest.pkl")
    with open(combined_out, "wb") as f:
        pickle.dump({"multiclass": model_multi, "binary": model_bin}, f)
    print(f"\nSaved combined models to {combined_out}")
    
    logging.info("Random forest baseline training complete.")
    print("\nDone!")
