"""
Stage 4 — Script 01: CIC-IDS2018 Multiclass Generalization Evaluation
======================================================================
Train:  CICIDS2017 (Stage 3 models)
Test:   CSE-CIC-IDS2018

Evaluates all 5 baseline models on the completely unseen CIC2018
...
logging.basicConfig(
    filename=os.path.join(LOGS_DIR, "01_cic2018_multiclass.log"),
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(message)s"
)

SAMPLE_SIZE = 500_000  # Stratified sample for efficient evaluation
...
def load_external_data(scaler, encoder):
    """Load, filter, sample, and scale the CIC2018 multiclass dataset."""
    data_path = os.path.join(STAGE2_ARTIFACTS, "cic2018_multiclass.parquet")
    print(f"\nLoading CIC2018 multiclass dataset from {data_path} ...")
    ...
    if missing:
        logging.error(f"Missing features in CIC2018: {missing}")
        sys.exit(1)
...
def evaluate_model(name, meta, X_scaled, y_str, encoder):
    ...
    # Classification report — derive labels from actual data
    ...
    report_path = os.path.join(TABLES_DIR, f"cic2018_multiclass_{name.lower().replace(' ', '_')}_report.csv")
    ...
    plt.title(f"CIC2018 Multiclass — {name}")
    plt.ylabel("True Label")
    plt.xlabel("Predicted Label")
    plt.tight_layout()
    fig_path = os.path.join(FIGURES_DIR, f"cm_cic2018_multi_{name.lower().replace(' ', '_')}.png")
...
def main():
    print("=" * 60)
    print(" STAGE 4: CIC2018 MULTICLASS GENERALIZATION EVALUATION ")
    print("=" * 60)
    ...
    out_path = os.path.join(TABLES_DIR, "cic2018_multiclass_results.csv")
    df_results.to_csv(out_path, index=False)
    print(f"\nResults saved to {out_path}")
    print("\n" + df_results.to_string(index=False))

    logging.info("Script 01 complete — CIC2018 multiclass evaluation finished.")
    print("\nDone!")
